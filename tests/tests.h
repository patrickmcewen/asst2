...
// Create our own test: recursively compute a factorial
class RecursiveFactorialTask : public IRunnable
{
public:
    int n;
    int *result;

    RecursiveFactorialTask(int n, int *result) : n(n), result(result) {}
 ...
 */
TestResults recursiveFactorialTestBase(ITaskSystem *t, bool do_async)
{
    int num_tasks = 256;
    int num_bulk_task_launches = 30;
    int factorial_num = 10; // Calculate 10!
    ...
    std::vector<RecursiveFactorialTask *> factorial_tasks(num_bulk_task_launches);
    for (int i = 0; i < num_bulk_task_launches; i++)
    {
        factorial_tasks[i] = new RecursiveFactorialTask(factorial_num, task_output);
    }
    ...
    else
    {
        for (int i = 0; i < num_bulk_task_launches; i++)
        {
            t->run(factorial_tasks[i], num_tasks);
        }
    }
    ...
    TestResults result;
    result.passed = true;
    int expected_factorial = 3628800; // The factorial of 10!
    for (int i = 0; i < num_tasks; i++)
    {
        if (task_output[i] != expected_factorial)
        {
            printf("%d\n", task_output[i]);
            result.passed = false;
            break;
    ...
    for (int i = 0; i < num_bulk_task_launches; i++)
    {
        delete factorial_tasks[i];
    }

    return result;
}

TestResults recursiveFactorialSyncTest(ITaskSystem *t)
{
    return recursiveFactorialTestBase(t, false);
}

TestResults recursiveFactorialAsyncTest(ITaskSystem *t)
{
    return recursiveFactorialTestBase(t, true);
}
